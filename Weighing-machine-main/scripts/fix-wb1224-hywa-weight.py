...
def main():
    ...
    print(f"  tare_image_path={row['tare_image_path']}")
    print(f"  camera_snapshots={row['camera_snapshots']}")

    vehicle = cur.execute(
        'SELECT vehicle_number, vehicle_type FROM vehicles WHERE vehicle_number = ?',
        (TRUCK,),
    ).fetchone()
    print('\nBefore vehicle:', dict(vehicle) if vehicle else None)
    ...
    after = cur.execute(
        ...
        'tare_image_path, camera_snapshots, ticket_status FROM transactions WHERE slip_number = ?',
        (SLIP,),
    ).fetchone()
    ...
    print('\nAfter transaction:')
    print(dict(after))
    print('\nAfter vehicle:', dict(vehicle_after) if vehicle_after else None)
    print(
